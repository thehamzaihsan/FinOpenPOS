...
-- Allow admins to see all records, others see only their own
...
CREATE POLICY "salesman_view_customers" ON customers
    FOR SELECT USING (
...
CREATE POLICY "salesman_view_products" ON products
    FOR SELECT USING (
        (SELECT role FROM users WHERE id = auth.uid()) IN ('admin', 'salesman')
    );
